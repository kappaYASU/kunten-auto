...
複雑な漢文を処理する場合に、ルーチンの中でループを繰り返していたところがあるので、それを少なくすると同時に、強調構文のように目的語が動詞の前に来る場合（代名詞が動詞の前に出る場合にはある程度対応していた）にもある程度は対応できるようにし、白文の漢文の漢字にx、zの記号を付すことによって、返読の禁止、読み飛ばしをして手動でも修正ができるようにしてみた。(2025/9/24)
